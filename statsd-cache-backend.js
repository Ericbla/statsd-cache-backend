...
var save_stats = function(callback) {
    if (loading) {
        // Prevent saving stats when loading stats is pending
        trace('statsd-cache-backend: save_stats: loading stats not finished');
        if (callback) {
            trace('statsd-cache-backend: save_stats: calling callback');
            callback();
        }
    } else {
...
var flush_stats = function (ts, metrics) {
    ...
    // Trick to detect the last flush done within the process exit handler
    var stack = new Error().stack;
    if (stack.indexOf('process.exit') !== -1) {
        trace('statsd-cache-backend: flush: Aborting on process exiting');
        return;
    }
    allStats.lastFlush = {
            'ts' : ts,
...
exports.init = function (startup_time, config, events, ready_callback) {
    ...
    if (typeof config.statsdCacheBackend.debug !== 'undefined') {
        debug = config.statsdCacheBackend.debug;
    }
    myBackendPort = config.statsdCacheBackend.httpPort || 123456;
